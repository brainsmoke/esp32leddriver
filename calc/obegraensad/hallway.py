...
def scale(arr):
    ...
    max_v = max(arr)
    arr = [ int(65535*v/max_v) for v in arr ]
    return arr

leds=get_leds()


manhattan = scale( -max(abs(x*1.5), abs(y)) for x, y, z in leds )
...
increments = [ 2, 3, 5, 7 ]
#groups = [ (x+y)%4 for y in range(16) for x in range(16) ]
groups = [ 2 for y in range(16) for x in range(16) ]

rotation_groups = [radar,y_rot,dist,x_rot]

rot = [ manhattan[i] for i in range(256) ]
