;
; ( [ ws2812 data pulses ] [ skipped pulse ] [ 4 bit number ] [ 3 bit mask ] )*
;
;  0         1                   ?                   ,              `
...
PIN_B = 6
PIN_C = 3
PIN_D = 0 ; pin changed from PA4 to PA0 on new pcb

RESET_COUNT_HIGH = 16
RESET_COUNT_LOW = 16

PIN_MASK = ((1<<PIN_A)|(1<<PIN_B)|(1<<PIN_C)|(1<<PIN_D))
...
.org 0x00
	...
	.macro wait_for_data ?loop, ?l1, ?l2, ?l3, ?l4
	...
	.macro relay IN, OUT1, OUT2, OUT3, ?sync, ?sync_loop, ?bit_start, ?read_one, ?l1, ?l2, ?l3, ?route_data, ?bit_fast_start
			mov a, #0
			clear count_forward
			inc count_forward
...
a_input:
relay PIN_A, PIN_B, PIN_C, PIN_D

b_input:
relay PIN_B, PIN_C, PIN_D, PIN_A

c_input:
relay PIN_C, PIN_D, PIN_A, PIN_B

d_input:
relay PIN_D, PIN_A, PIN_B, PIN_C
